...
import { SEARCH_FIELD, keyboardHideClass, keyboardClass } from "../magicStrings.js";
...
const buttons = [["Backquote", "`", "ё", "~"], ["Digit1", "1", "1", "!"], ["Digit2", "2", "2", "@", "\""],
                    ...
                    ["Period", ".", "ю", ">"], ["Slash", "/", ".", "?", ","],
                    ["lang", "EN/RU", "RU/EN"],["Space", "Change language : Shift + Alt", "Переключение языка : Shift + Alt", "", ""], ["ArrowLeft", "&#8592;",  "&#8592;"], 
                    ["ArrowRight", "&#8594;", "&#8594;"]
                    ];
...
const EDITOR = document.querySelector('#search-string');
    ...
    if (event.target.classList.value === 'keybord') {
        ...
        switch (event.target.id) {
            ...
            case enter:
                ...
                KEYBORD.classList.toggle(keyboardHideClass);
                break;
            case arrowLeft:
                cursorLeftRight(-1);
                break;
...
export function showKeyboard() {
    document.querySelector(keyboardClass).classList.toggle(keyboardHideClass);
}

function createPage() {
    document.body.insertAdjacentHTML('afterbegin',
    `<div class="keybord keybord-hide"></div>`);
}
...
function createKeybord() {
    ...
    for (let i = 0; i < buttons.length; i++) {
        KEYBORD.insertAdjacentHTML('beforeend', '<div class="button-keyboard" id="' + buttons[i][0] + '">' + buttonContent(buttons[i][0], '') + '</div>');
    }
}
